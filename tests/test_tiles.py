...
tile_test_cases: List[TileTestCase] = [
    TileTestCase(
        ...
        tile_args=[0.2, 0.5, {"command": "echo hi", "id": 1}],
        expected_tile=tiles.Tile(
            width=20,
            height=50,
            window=windows.Window(id=1, command="echo hi", width=20, height=50),
        ),
    ),
    TileTestCase(
        screen_dimensions=tiles.ScreenDimensions(100, 100),
        tile_args=[0.33, 0.66, {"command": "echo bye", "id": 5}],
        expected_tile=tiles.Tile(
            width=33,
            height=66,
            window=windows.Window(id=5, command="echo bye", width=33, height=66),
        ),
    ),
    TileTestCase(
        screen_dimensions=tiles.ScreenDimensions(1000, 1000),
        tile_args=[0.33, 0.554, {"command": "cowsay moo", "id": 5}],
        expected_tile=tiles.Tile(
            width=330,
            height=554,
            window=windows.Window(id=5, command="cowsay moo", width=330, height=554),
        ),
    ),
]
